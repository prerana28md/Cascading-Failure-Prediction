    ...
    public boolean preHandle(HttpServletRequest req, HttpServletResponse res, Object handler) throws Exception {
        ...
        switch (faultState.getActiveFault()) {
            ...
            case ERROR   -> { res.setStatus(500); res.setContentType("application/json");
                res.getWriter().write("{\"error\":\"Injected fault: SERVICE_ERROR\",\"service\":\"shipping-service\"}"); return false; }
            case DOWN    -> { res.setStatus(503); res.setContentType("application/json");
                res.getWriter().write("{\"error\":\"Injected fault: SERVICE_DOWN\",\"service\":\"shipping-service\"}"); return false; }
            default -> {}
        }
        return true;
    }
