...
public class FaultController {
    ...
    public ResponseEntity<Map<String, Object>> configure(@RequestBody Map<String, Object> body) {
        ...
        return ResponseEntity.ok(Map.of("service","inventory-service","fault",fault.name(),"delayMs",delayMs,"status","APPLIED"));
    }

    @PostMapping("/reset")
    public ResponseEntity<Map<String, Object>> reset() {
        faultState.reset();
        return ResponseEntity.ok(Map.of("service","inventory-service","status","RESET"));
    }

    @GetMapping("/status")
    public ResponseEntity<Map<String, Object>> status() {
        return ResponseEntity.ok(Map.of("service","inventory-service","fault",faultState.getActiveFault().name(),"delayMs",faultState.getDelayMs()));
    }
}
